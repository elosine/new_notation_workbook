...
var dial = mkDialNO(0, 500, 500, 50, 5, 12, 111);
...
function mkDialNO(ix, w, h, x, y, numTicks, bpm) {
  ...
  var availableNotes = [
    "accent.svg",
    "eight_accent_2ndPartial.svg",
    "quarter_accent.svg",
    "triplet_accent.svg"
  ];
  for (var i = 0; i < tickDegs.length; i++) {
    var useNotation = probability(0.5);
    if (useNotation) {
      var useNote = choose(availableNotes);
      var rectx = midRadius * Math.cos(rads(tickDegs[i])) + cx - (rectSize / 2);
      var recty = midRadius * Math.sin(rads(tickDegs[i])) + cy - (rectSize / 2);
      var noteBox = document.createElementNS(SVG_NS, "rect");
      noteBox.setAttributeNS(null, "width", rectSize);
      noteBox.setAttributeNS(null, "height", rectSize);
      noteBox.setAttributeNS(null, "transform", "translate( " + rectx.toString() + "," + recty.toString() + ")");
      ...
      notationSVG.setAttributeNS(SVG_XLINK, 'xlink:href', "/notation/" + useNote);
      notationSVG.setAttributeNS(null, 'width', rectSize);
      // var t_svgH = notationContH * 0.6666666667;
      notationSVG.setAttributeNS(null, 'height', rectSize);
